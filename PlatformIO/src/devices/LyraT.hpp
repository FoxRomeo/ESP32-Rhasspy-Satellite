 ...
 */
void LyraT::updateColors(StateColors colors)
{
  switch (colors) {
  case COLORS_HOTWORD:
...
void LyraT::InitI2SSpeakerOrMic(int mode)
{
...
void LyraT::setWriteMode(int sampleRate, int bitDepth, int numChannels)
{
  if (mode != MODE_SPK) {
...
void LyraT::writeAudio(uint8_t *data, size_t size, size_t *bytes_written)
{
...
bool LyraT::readAudio(uint8_t *data, size_t size)
{
  size_t byte_read;
...
void LyraT::muteOutput(bool mute)
{
 ...
 * @param volume 0 - 100
 */
void LyraT::setVolume(uint16_t volume)
{
  out_vol = volume;
 ...
 */
bool LyraT::isHotwordDetected()
{
  // TODOD debounce maybe?
  return digitalRead(key_listen) == LOW;
}

void LyraT::ampOutput(int ampOut)
{
...
void LyraT::updateBrightness(int brightness)
{
